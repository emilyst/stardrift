...
#[path = "../src/physics/octree.rs"]
pub mod octree;

fn generate_test_bodies(count: usize, seed: u64) -> Vec<octree::OctreeBody> {
...
fn bench_octree_construction(c: &mut Criterion) {
    ...
    for &count in &body_counts {
        ...
        group.bench_with_input(BenchmarkId::new("bodies", count), &count, |b, _| {
            b.iter(|| {
                let mut octree = octree::Octree::new(theta, min_distance, max_force);
...
fn bench_octree_pool_reuse(c: &mut Criterion) {
    ...
    for &count in &body_counts {
        ...
        group.bench_with_input(BenchmarkId::new("without_pool", count), &count, |b, _| {
            b.iter(|| {
                let mut octree1 = octree::Octree::new(theta, min_distance, max_force);
                octree1.build(black_box(bodies.clone()));
                black_box(&octree1);

                let mut octree2 = octree::Octree::new(theta, min_distance, max_force);
                octree2.build(black_box(bodies.clone()));
                black_box(&octree2);

                let mut octree3 = octree::Octree::new(theta, min_distance, max_force);
        ...
        group.bench_with_input(BenchmarkId::new("with_pool", count), &count, |b, _| {
            b.iter(|| {
                let mut octree =
                    octree::Octree::with_pool_capacity(theta, min_distance, max_force, 50, 100);
